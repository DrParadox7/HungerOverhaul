...
public class ModuleBonemeal {
    private static HashMap<Class<? extends Block>, BonemealModification> bonemealModificationsByBlockClass =
            new HashMap<Class<? extends Block>, BonemealModification>();
    private static HashMap<Block, BonemealModification> bonemealModificationsByBlock =
            new HashMap<Block, BonemealModification>();

    public static void registerBonemealModifier(
            Class<? extends Block> blockClass, BonemealModification bonemealModification) {
    ...
    public static BonemealModification getBonemealModification(Block block) {
        ...
        if (bonemealModification != null) return bonemealModification;
        else return getBonemealModification(block.getClass());
    }
    ...
    public static BonemealModification getBonemealModification(Class<? extends Block> blockClass) {
        ...
        if (bonemealModification == null) {
            for (Map.Entry<Class<? extends Block>, BonemealModification> entry :
                    bonemealModificationsByBlockClass.entrySet()) {
                if (entry.getKey().isInstance(blockClass)) return entry.getValue();
            }
        }
    ...
    public void onBonemealUsed(BonemealEvent event) {
        ...
        if (bonemealModification == null) return;
        ...
        if (event.world.rand.nextFloat() < IguanaConfig.bonemealEffectiveness) {
            if (IguanaConfig.modifyBonemealGrowth) {
                ...
                int resultingMeta =
                        bonemealModification.getNewMeta(event.world, event.x, event.y, event.z, event.block, meta);
