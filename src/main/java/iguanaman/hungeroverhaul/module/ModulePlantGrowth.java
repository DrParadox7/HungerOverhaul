...
public class ModulePlantGrowth {
    private static HashMap<Class<? extends Block>, PlantGrowthModification> plantGrowthModificationsByBlockClass =
            new HashMap<Class<? extends Block>, PlantGrowthModification>();
    private static HashMap<Block, PlantGrowthModification> plantGrowthModificationsByBlock =
            new HashMap<Block, PlantGrowthModification>();

    public static void registerPlantGrowthModifier(
            Class<? extends Block> blockClass, PlantGrowthModification growthModification) {
    ...
    public static PlantGrowthModification getPlantGrowthModification(Block block) {
        ...
        if (growthModification != null) return growthModification;
        else return getPlantGrowthModification(block.getClass());
    }
    ...
    public static PlantGrowthModification getPlantGrowthModification(Class<? extends Block> blockClass) {
        ...
        if (growthModifier == null) {
            for (Map.Entry<Class<? extends Block>, PlantGrowthModification> entry :
                    plantGrowthModificationsByBlockClass.entrySet()) {
                if (entry.getKey().isAssignableFrom(blockClass)) return entry.getValue();
            }
        }
    ...
    public void allowGrowthTick(PlantGrowthEvent.AllowGrowthTick event) {
        ...
        if (growthModification == null) return;

        // sunlight
        float sunlightModifier = !growthModification.needsSunlight
                        || (growthModification.needsSunlight
                                && event.world.isDaytime()
                                && event.world.canBlockSeeTheSky(event.x, event.y, event.z))
                ? 1
                : IguanaConfig.noSunlightRegrowthMultiplier;
        ...
        // random
        if (RandomHelper.nextFloat(
                        event.random, growthModification.growthTickProbability * biomeModifier * sunlightModifier)
                >= 1) {
            event.setResult(Result.DENY);
            return;
        }
