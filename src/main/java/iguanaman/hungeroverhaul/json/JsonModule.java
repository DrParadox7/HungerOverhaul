...
public class JsonModule {
    ...
    public static void preinit(File configFolder) {
        ...
        if (!hoFolder.exists()) hoFolder.mkdirs();

        for (File potentialConfigFile : hoFolder.listFiles()) {
            if (!FilenameUtils.getExtension(potentialConfigFile.getName()).equalsIgnoreCase("json")) continue;
    ...
    public static void addGameObjectToList(ItemAndBlockList list, GameObject gameObj) {
        if (gameObj.name.isEmpty()) return;

        if (gameObj.meta == OreDictionary.WILDCARD_VALUE) {
            if (gameObj.toBlock() != null) list.add(gameObj.toBlock());
            if (gameObj.toItem() != null) list.add(gameObj.toItem());
        } else {
