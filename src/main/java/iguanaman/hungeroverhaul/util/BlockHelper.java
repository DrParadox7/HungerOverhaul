...
public class BlockHelper {
    public static List<ItemStack> modifyCropDrops(
            List<ItemStack> drops, Block block, int meta, int minSeeds, int maxSeeds, int minProduce, int maxProduce) {
        ...
        boolean produceIsNotSeed = (seedItem.getItem() != produceItem.getItem()
                || seedItem.getItemDamage() != produceItem.getItemDamage());
        ...
        if (produceIsNotSeed && seedItem.stackSize > 0) modifiedDrops.add(seedItem);

        if (produceItem.stackSize > 0) modifiedDrops.add(produceItem);
    ...
    public static Item getSeedItem(Block block, int meta) {
        ...
        if (Loader.isModLoaded("harvestcraft") && block instanceof BlockPamCrop) {
            ...
            if (seedForProduct != null) return seedForProduct;
        }
        return itemDropped;
    }
