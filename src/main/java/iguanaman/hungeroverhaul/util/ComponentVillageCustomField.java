...
public class ComponentVillageCustomField extends StructureVillagePieces.Village {
    ...
    public ComponentVillageCustomField(
            Start par1ComponentVillageStartPiece,
            int par2,
            Random par3Random,
    ...
    private Block getRandomCropHarvestCraft(Random random) {
        ...
        return random.nextInt(length + 3) <= length
                ? PamsModsHelper.PamCrops[random.nextInt(length)]
                : getRandomCropVanilla(random);
    }
    ...
    public static ComponentVillageCustomField buildComponent(
            Start par0Start, List par1List, Random par2Random, int par3, int par4, int par5, int par6, int par7) {
        ...
        return canVillageGoDeeper(var8) && StructureComponent.findIntersecting(par1List, var8) == null
                ? new ComponentVillageCustomField(par0Start, par7, par2Random, var8, par6)
                : null;
    }
    ...
    public boolean addComponentParts(
        ...
        if (typeA == fieldType.REED) {
            if (BiomeDictionary.isBiomeOfType(biome, Type.SANDY)) blockOutA = Blocks.sand;
            else blockOutA = Blocks.dirt;
            crop1A = Blocks.reeds;
            crop2A = Blocks.reeds;
        ...
        if (typeB == fieldType.REED) {
            if (BiomeDictionary.isBiomeOfType(biome, Type.SANDY)) blockOutB = Blocks.sand;
            else blockOutB = Blocks.dirt;
            crop1B = Blocks.reeds;
            crop2B = Blocks.reeds;
        } else if (typeA == fieldType.STEM) {
        ...
        for (int tmp = 1; tmp <= 7; tmp++) {
            if (typeA == fieldType.NORMAL)
                placeCropAtCurrentPosition(par1World, crop1A, cropMeta1A, 1, 1, tmp, par3StructureBoundingBox);
            ...
            if (typeA == fieldType.NORMAL)
                placeCropAtCurrentPosition(par1World, crop2A, cropMeta2A, 5, 1, tmp, par3StructureBoundingBox);
            if (typeA == fieldType.NORMAL)
                placeCropAtCurrentPosition(par1World, crop1B, cropMeta1B, 7, 1, tmp, par3StructureBoundingBox);
            ...
            if (typeA == fieldType.NORMAL)
                placeCropAtCurrentPosition(par1World, crop2B, cropMeta2B, 11, 1, tmp, par3StructureBoundingBox);
        }
    ...
    private void placeCropAtCurrentPosition(
            World world, Block block, int meta, int offsetX, int offsetY, int offsetZ, StructureBoundingBox box) {
        if (Loader.isModLoaded("harvestcraft"))
            placeHarvestCraftCropAtCurrentPosition(world, block, meta, offsetX, offsetY, offsetZ, box);
        else placeBlockAtCurrentPosition(world, block, meta, offsetX, offsetY, offsetZ, box);
    }

    private void placeHarvestCraftCropAtCurrentPosition(
            World world, Block block, int meta, int offsetX, int offsetY, int offsetZ, StructureBoundingBox box) {
