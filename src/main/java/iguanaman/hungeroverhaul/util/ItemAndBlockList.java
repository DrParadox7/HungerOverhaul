...
public class ItemAndBlockList {
    ...
    public void add(ItemStack itemStack) {
        if (itemStack != null && itemStack.getItem() != null) itemStacks.add(itemStack);
    }
    ...
    public boolean contains(Class<?> clazz) {
        ...
        while (itr.hasNext()) {
            ...
            if (testClass.isAssignableFrom(clazz)) return true;
        }
    ...
    public boolean contains(ItemStack itemStack) {
        if (itemStack != null && itemStack.getItem() != null) {
            for (ItemStack curItemStack : itemStacks) {
                if (OreDictionary.itemMatches(curItemStack, itemStack, false)) return true;
            }
        }
        return false;
    }
    ...
    public boolean contains(Item item) {
        ...
        if (contains(new ItemStack(item))) return true;

        if (contains(item.getClass())) return true;
    ...
    public boolean contains(Block block) {
        ...
        if (contains(new ItemStack(block))) return true;

        if (contains(block.getClass())) return true;
