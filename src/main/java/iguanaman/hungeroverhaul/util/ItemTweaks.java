...
public class ItemTweaks {
        ...
        if (worthLooping) {
            for (Object obj : Item.itemRegistry) {
                ...
                if (item instanceof ItemFood) {
                    ...
                    if (IguanaConfig.modifyFoodStackSize) modifyStackSize(item, stack, values);
                    if (values.hunger > 9) {
                        if (IguanaConfig.addTradesButcher) addButcherTrade(item, stack, values);
                        if (IguanaConfig.addHarvestCraftChestLoot) addChestLoot(item, stack, values);
                    }
                    if (Loader.isModLoaded("harvestcraft")
                            && IguanaConfig.addTradesFarmer
                            && item instanceof ItemPamSeedFood) {
                        addCropTrade(item);
                    }
                } else if (Loader.isModLoaded("harvestcraft")
                        && IguanaConfig.addSaplingTradesFarmer
                        && item instanceof ItemBlock) {
                    Block block = Block.getBlockFromItem(item);
                    if (block instanceof BlockPamSapling) addSaplingTrade(block);
                }
            }
        }
    }
    ...
    public static void modifyStackSize(Item item, ItemStack stack, FoodValues values) {
        ...
        if (values.hunger <= 2) newStackSize = 16 * IguanaConfig.foodStackSizeMultiplier;
        else if (values.hunger <= 5) newStackSize = 8 * IguanaConfig.foodStackSizeMultiplier;
        else if (values.hunger <= 8) newStackSize = 4 * IguanaConfig.foodStackSizeMultiplier;
        else if (values.hunger <= 11) newStackSize = 2 * IguanaConfig.foodStackSizeMultiplier;
        else newStackSize = IguanaConfig.foodStackSizeMultiplier;

        if (curStackSize > newStackSize) item.setMaxStackSize(newStackSize);
    }

    public static void addButcherTrade(Item item, ItemStack stack, FoodValues values) {
        VillagerRegistry.instance()
                .registerVillageTradeHandler(
                        VillagerType.BUTCHER.ordinal(), new TradeHandlerFood(item, item.getItemStackLimit(stack)));
    }

    public static void addSaplingTrade(Block block) {
        VillagerRegistry.instance()
                .registerVillageTradeHandler(VillagerType.FARMER.ordinal(), new TradeHandlerSapling(block));
    }

    public static void addCropTrade(Item item) {
        VillagerRegistry.instance()
                .registerVillageTradeHandler(VillagerType.FARMER.ordinal(), new TradeHandlerCrop(item));
    }
    ...
    public static void addChestLoot(Item item, ItemStack stack, FoodValues values) {
        ...
        ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST)
                .addItem(new WeightedRandomChestContent(stack, 1, maxStackSize, IguanaConfig.chestLootChance));
