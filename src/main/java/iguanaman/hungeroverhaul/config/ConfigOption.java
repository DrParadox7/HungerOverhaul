...
public class ConfigOption<T> {
    ...
    public ConfigOption(
            String category, String name, T defaultValue, T minValue, T maxValue, T blankSlate, String comment) {
    ...
    public T get(Configuration config) {
        if (defaultValue instanceof Boolean)
            ...
            return (T) Integer.valueOf(config.getInt(
                    name, category, (Integer) defaultValue, (Integer) minValue, (Integer) maxValue, getComment()));
        else if (defaultValue instanceof Float)
            return (T) Float.valueOf(config.getFloat(
                    name, category, (Float) defaultValue, (Float) minValue, (Float) maxValue, getComment()));
        else if (defaultValue instanceof Double)
            return (T) Double.valueOf(Math.min(
                    (Double) maxValue,
                    Math.max((Double) minValue, getProperty(config).getDouble())));
        else if (defaultValue instanceof String)
            return (T) config.getString(name, category, (String) defaultValue, getComment());
        else
            throw new RuntimeException("Unknown ConfigOption type for '" + category + ":" + name + "': "
