...
public class FoodEventHandler {
    ...
    public void onFoodEaten(FoodEvent.FoodEaten event) {
        ...
        if (IguanaConfig.foodRegensHealth) {
            ...
            if (toHeal > 0f) event.player.heal(toHeal);
        }
    }
    ...
    public void getMaxExhaustion(ExhaustionEvent.GetMaxExhaustion event) {
        ...
        if (IguanaConfig.difficultyScalingHunger) {
            if (difficulty == EnumDifficulty.PEACEFUL) hungerLossRate *= 5F / 3F;
            else if (difficulty == EnumDifficulty.EASY) hungerLossRate *= 4F / 3F;
            else if (IguanaConfig.harderScalingHunger && difficulty == EnumDifficulty.HARD) hungerLossRate *= 2F / 3F;
        }
    ...
    public void onExhausted(ExhaustionEvent.Exhausted event) {
        ...
        if (event.player.getFoodStats().getSaturationLevel() <= 0) event.deltaHunger = -1;
    }
    ...
    public void onHealthRegen(HealthRegenEvent.Regen event) {
        if (IguanaConfig.disableHealingHungerDrain) event.deltaExhaustion = 0;
    }
    ...
    public void onFoodStatsAddition(FoodEvent.FoodStatsAddition event) {
        if (IguanaConfig.hungerLossRatePercentage == 0) event.setCanceled(true);
    }
    ...
    public void onFoodStartEating(PlayerUseItemEvent.Start event) {
        if (IguanaConfig.modifyFoodEatingSpeed && AppleCoreAPI.accessor.isFood(event.item)) {
            ...
            if (hunger > 0) event.duration = hunger * 8 + 8;
        }
    }
}
